...
export default class ProductTypesEdit extends Vue {
    ...
    async mounted(): Promise<void> {
        try {
            ...
            this.createdAt = format(new Date(productType.createdAt), "yyyy-MM-dd'T'HH:mm");
        } catch (e) {
            console.error(`Error retrieving productType data: ${e}`)
        }
    ...
    async Update(): Promise<void> {
        if (this.name) {
            var res = await this.productTypesService.put(this.id, {
                id: this.id,
                name: this.name,
